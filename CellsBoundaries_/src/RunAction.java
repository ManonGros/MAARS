...
public class RunAction implements ActionListener{
	...
	public RunAction (CellsBoundaries_ cB) {
		this.cB = cB;
	 ...
	 * To choose if you want to convert width or height you can use CellsBoundaries_ constants : WIDTH and HEIGHT and DEPTH.
	...
	public void checkUnitsAndScale(){
		...
		if (cB.getImageToAnalyze().getCalibration().scaled()){
			...
			if (cB.getImageToAnalyze().getCalibration().getUnit().equals("micron") || cB.getImageToAnalyze().getCalibration().getUnit().equals("µm")) {
				...
				scale[CellsBoundaries_.WIDTH] = cB.getImageToAnalyze().getCalibration().pixelWidth;
				scale[CellsBoundaries_.HEIGHT] = cB.getImageToAnalyze().getCalibration().pixelHeight;
				scale[CellsBoundaries_.DEPTH] = cB.getImageToAnalyze().getCalibration().pixelDepth;
	...
	public void changeScale(int maxWidth, int maxHeight){
		...
		if (cB.getImageToAnalyze().getWidth() > maxWidth) {
        	...
        	if (cB.getImageToAnalyze().getCalibration().scaled()) {
	        	...
	        	newCal.pixelWidth = cB.getScale()[CellsBoundaries_.WIDTH]*cB.getImageToAnalyze().getWidth()/maxWidth;
	        	newCal.pixelHeight = cB.getScale()[CellsBoundaries_.HEIGHT]*cB.getImageToAnalyze().getWidth()/maxWidth;
	        	newCal.pixelDepth = cB.getScale()[CellsBoundaries_.DEPTH];
        	}
        	...
        	if (newHeight > maxHeight) {
            	...
            	if (cB.getImageToAnalyze().getCalibration().scaled()) {
	            	newCal.pixelWidth = cB.getScale()[CellsBoundaries_.WIDTH]*cB.getImageToAnalyze().getHeight()/maxHeight;
	            	newCal.pixelHeight = cB.getScale()[CellsBoundaries_.HEIGHT]*cB.getImageToAnalyze().getHeight()/maxHeight;
	            	newCal.pixelDepth = cB.getScale()[CellsBoundaries_.DEPTH];
            	}
		...
		if (fileNameField.getText().isEmpty()) {
			...
			if (!thereIsAResult) {
				...
				if (cB.getSizeField().getText().isEmpty()) {
					...
					if(CellsBoundaries_.MICRONS == cB.getSizeComUnit().getSelectedIndex()
							|| CellsBoundaries_.MICRONS == cB.getMaxWComboUnit().getSelectedIndex()
							|| CellsBoundaries_.MICRONS == cB.getMaxHComboUnit().getSelectedIndex()
							|| CellsBoundaries_.MICRONS == cB.getMinParticleSizeComboUnit().getSelectedIndex()
							|| CellsBoundaries_.MICRONS == cB.getMaxParticleSizeComboUnit().getSelectedIndex()) {
						checkUnitsAndScale();
					}
					...
					if (typicalCellSize <= 0){
						...
						if (CellsBoundaries_.MICRONS == cB.getSizeComUnit().getSelectedIndex() && unitsChecked) {
							sigma = convertMicronToPixelSize(typicalCellSize, CellsBoundaries_.DEPTH);
							System.out.println("typical size is in micron, convert it in pixels : "+sigma);
						}
						else  {
							if (CellsBoundaries_.PIXELS == cB.getSizeComUnit().getSelectedIndex()){
						...
						if (minParticleSizeTemp <= 0){
							...
							if (CellsBoundaries_.MICRONS == cB.getMinParticleSizeComboUnit().getSelectedIndex() && unitsChecked) {
								minParticleSize = minParticleSizeTemp*convertMicronToPixelSize(1, CellsBoundaries_.WIDTH)*convertMicronToPixelSize(1, CellsBoundaries_.HEIGHT);
								System.out.println("Cell Area is in micron, convert it in pixels : "+minParticleSize);
							}
							else  {
								if (CellsBoundaries_.PIXELS == cB.getMinParticleSizeComboUnit().getSelectedIndex()){
							...
							if (maxParticleSizeTemp <= 0){
								...
								if (CellsBoundaries_.MICRONS == cB.getMaxParticleSizeComboUnit().getSelectedIndex() && unitsChecked) {
									maxParticleSize = maxParticleSizeTemp*convertMicronToPixelSize(1, CellsBoundaries_.WIDTH)*convertMicronToPixelSize(1, CellsBoundaries_.HEIGHT);
									System.out.println("Cell Area is in micron, convert it in pixels : "+maxParticleSize);
								}
								else  {
									if (CellsBoundaries_.PIXELS == cB.getMaxParticleSizeComboUnit().getSelectedIndex()){
								...
								if (cB.getScaleCkb().getState()){
									...
									if (newMaxWidth <= 0){
										...
										if (newMaxHeight <= 0){
											...
											if(CellsBoundaries_.MICRONS == cB.getMaxWComboUnit().getSelectedIndex() && unitsChecked) {
												
												maxWidth = convertMicronToPixelSize(newMaxWidth, CellsBoundaries_.WIDTH);
												System.out.println("Width value is in micron, convert it in pixel : "+maxWidth);
											}
											else {
												if (CellsBoundaries_.PIXELS == cB.getMaxWComboUnit().getSelectedIndex()){
													maxWidth =  (int) newMaxWidth;
												}
											}
											if(CellsBoundaries_.MICRONS == cB.getMaxHComboUnit().getSelectedIndex() && unitsChecked){
												
												maxHeight = convertMicronToPixelSize(newMaxHeight, CellsBoundaries_.HEIGHT);
												System.out.println("Height value is in micron, convert it in pixel : "+maxHeight);
											}
											else {
												if(CellsBoundaries_.PIXELS == cB.getMaxHComboUnit().getSelectedIndex()){
